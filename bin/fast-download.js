...
program
  ...
  .option(
    '-f, --filename <s>',
    'destination filename (default: base filename in url)'
  )
  ...
  .option(
    '-s --chunkSize <n>',
    'the size of a data chunk in MB (default: 0, meaning file size divided by chunksAtOnce)',
    parseFloat
  )
  .option(
    '-t, --timeout',
    'timeout on http requests in seconds (default: 0, meaning no timeout)',
    parseInt
  )
  .option(
    '-w, --width <n>',
...
if (urls.length > 1 && options.filename) {
  throw new Error(
    'Can not download multiple urls to a single file. Remove the --filename directive.'
  )
}
FastDownload.defaultOptions.resumeFile = !options.overwrite
FastDownload.defaultOptions.chunksAtOnce = options.chunksAtOnce
FastDownload.defaultOptions.chunkSize = Math.floor(
  options.chunkSize * 1024 * 1024
)
if (options.timeout) {
  FastDownload.defaultOptions.timeout = options.timeout * 1000
}
      ...
      var dl = new FastDownload(url, { destFile: destination, }, function (error) {
        ...
        console.log(
          '    ' +
            ...
            destination.bold +
            ' (' +
            Math.floor(dl.file_size / 1024 / 1024 * 100) / 100 +
            'MB)'
        )
