...
FastDownload.prototype._init_http = function (cb) {
  ...
  if (!acceptRanges) {
    if (self._options.end.toString() != self.file_size.toString()) {
      cb (new Error('the server will not accept range requests, so cannot change "end" of file'))
    }

    if (self._options.start !== 0) {
